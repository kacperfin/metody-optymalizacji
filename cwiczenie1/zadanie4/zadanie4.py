from pathlib import Path
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from scipy.optimize import linprog

script_dir = Path(__file__).parent
data = pd.read_csv(script_dir.parent / 'data01.csv', header=None)

x = data[0].values
y = data[1].values
N = len(x)

Phi = np.column_stack((x, np.ones(N)))

# Suma kwadratów różnic (LS)

theta_LS = np.linalg.pinv(Phi) @ y # pinv to wyznaczenie macierzy pseudoodwrotnej
a_LS, b_LS = theta_LS

print(f"Suma kwadratów różnic (LS): a = {a_LS:.4f}, b = {b_LS:.4f}")

y_LS = a_LS * x + b_LS # predykcja metodą LS

# Suma wartości bezwzględnych różnic (LP)

c = np.zeros(N + 2)
c[2:] = 1

A_ub = np.vstack([
    np.hstack([Phi, -np.eye(N)]),
    np.hstack([-Phi, -np.eye(N)])
])
b_ub = np.hstack([y, -y])

res = linprog(c, A_ub=A_ub, b_ub=b_ub, bounds=[(None, None), (None, None)] + [(0, None)] * N) # bounds określa zakresy dla N+2 szukanych zmiennych: parametry a i b mogą być dowolne (None, None), a błędy u_i muszą być >= 0 (0, None)

if res.success:
    theta_LP = res.x[:2]
    a_LP, b_LP = theta_LP
    print(f"Suma wartości bezwzględnych różnic (LP): a = {a_LP:.4f}, b = {b_LP:.4f}")

    y_LP = a_LP * x + b_LP # predykcja metodą LP
else:
    print("Niepowodzenie LP: ", res.message)

# Wizualizacja

plt.figure(figsize=(10, 6))
plt.scatter(x, y, color='red', s=10, label='Punkty')

plt.plot(x, y_LS, color='black', linewidth=2, label=f'LS: y = {a_LS:.2f}x + {b_LS:.2f}')

if res.success:
    plt.plot(x, y_LP, color='blue', linewidth=2, label=f'LP: y = {a_LP:.2f}x + {b_LP:.2f}')

plt.xlabel('x')
plt.ylabel('y')
plt.legend()
plt.grid(True)

plots_dir = script_dir / 'wykresy'
plots_dir.mkdir(exist_ok=True)

plt.ylim(-50, 150)
plt.savefig(plots_dir / 'zadanie4-001.png')
plt.ylim(-10, 15)
plt.savefig(plots_dir / 'zadanie4-002.png')